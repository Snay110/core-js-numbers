...
function getAverage(value1, value2) {
  const svg = (value1 + value2) / 2;
  return svg === Infinity ? Number.MAX_VALUE : svg;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const aaa = x1 * x2 + y1 * y2;
  const sum = Math.sqrt(x1 ** 2 + y1 ** 2);
  const sum2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(aaa / (sum * sum2));
}
...
function roundToPowerOfTen(num, pow) {
  if (typeof num !== 'number' || typeof pow !== 'number') {
    throw new Error('аргументы должны быть чистыми ');
  }
...
function toNumber(value, def) {
  ...
  if (value instanceof Number) {
    return Number.isNaN(value.valueOf()) ? def : value.valueOf();
  }
